...
export function ExamPage({ session, questionsById, onStart, onSelect, onJump, onReview, onSubmit }: ExamPageProps) {
  ...
  return <div className="page exam-page"><ExamHeader expiresAt={session.expiresAt} current={safeIndex + 1} total={session.questionIds.length} onExpire={onSubmit} /><div className="exam-layout"><div><QuestionCard question={question} mode="exam" selectedOptionIds={session.answers[question.id] ?? []} onChange={(optionId) => onSelect(question.id, optionId)} /><div className="exam-actions"><button className={session.reviewQuestionIds.includes(question.id) ? 'review-toggle active' : 'review-toggle'} onClick={() => onReview(question.id)}>{session.reviewQuestionIds.includes(question.id) ? 'Marked for review' : 'Mark for review'}</button><div><button className="button ghost" disabled={safeIndex === 0} onClick={() => jump(safeIndex - 1)}>Previous</button><button className="button primary" onClick={() => safeIndex === session.questionIds.length - 1 ? onSubmit() : jump(safeIndex + 1)}>{safeIndex === session.questionIds.length - 1 ? 'Submit exam' : 'Next question'}</button></div></div></div><ExamNavigator questionIds={session.questionIds} currentIndex={safeIndex} answeredIds={answeredIds} reviewIds={session.reviewQuestionIds} onJump={jump} /></div><button className="submit-all" onClick={onSubmit}>Submit exam</button></div>
}
